function [bw_closed] = close_boundary(im_bw)
indexes = find(im_bw);
stacked_boundaries = {};
...
% while (c < l+1)
%     list = stacked_boundaries{1, i}.list;
%     if length(list)==1
%          stacked_boundaries{1, i} = [];
%     else
%         i = i+1;
%     end
%     c = c+1;
...
im_bounds_bw = get_colored_image_of_borders(im_bw, stacked_boundaries);
bw_closed = join_boundaries(im_bounds_bw, stacked_boundaries, additional_ending_leaves);
