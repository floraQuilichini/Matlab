...
while i < length(stacked_boundaries)
    ...
    for k = i+1:length(stacked_boundaries)
        ...
        if (any(n1_next(:) == ee1 ))
            ...
            additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, k}.leaf_init) = []; 
            additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, i}.leaf_init) = []; 
        ...
        if (any(n2_next(:) == ee1))
            ...
            additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, k}.leaf_init) = []; 
            additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, i}.leaf_end) = []; 
            stacked_boundaries{1, i}.list = list;
        ...
        if (any(n1_next(:) ==ee2 ))
            ...
            additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, k}.leaf_end) = []; 
            additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, i}.leaf_init) = []; 
            stacked_boundaries{1, i}.list = list;
        ...
        if length(prev_i)>1
            ...
            for m=1:length(prev_i)
                if (ee1 ==prev_i(m))
                    ...
                    additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, k}.leaf_init) = [];
                    additional_ending_leaves(additional_ending_leaves == stacked_boundaries{1, i}.leaf_end) = [];
                    additional_ending_leaves(additional_ending_leaves == prev_i(m)) = [];
                    stacked_boundaries{1, i}.list = list;
                    stacked_boundaries{1, i}.leaf_end = stacked_boundaries{1, k}.leaf_end;
                    stacked_boundaries{1, i}.prev = stacked_boundaries{1, k}.prev;
                    stacked_boundaries(k) = [];
