...
for k = 1:size(unclosed_endings, 1)
    extremities = unclosed_endings(k, :);
    for i=1:2
        [dist, border_index] = compute_shortest_distance_to_image_border(sz, extremities(i));
        if dist < max_dist
            [row, col] = ind2sub(sz, extremities(i));
            [closest_point_row, closest_point_col] = ind2sub(sz, border_index);
            bw_closed = linept(bw_closed, row, col, closest_point_row, closest_point_col);
        end
    end
end
