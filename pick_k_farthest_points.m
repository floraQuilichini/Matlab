function k_farthest_points = pick_k_farthest_points(points, first_point, k, varargin)
% points : n-by-3 array
...
% dist_to_all_points = 0.0;
% working_points = selected_points;
% % compute distance between points
% while (size(working_points, 1) > 1)
%     dist_to_all_points = dist_to_all_points + sum(working_points(1:end-1, :)*working_points(end, :)');
%     working_points = working_points(1:end-1, :);
% end 
...
dists_max_points_to_all_points = sum(l2_dists_points_to_all_points);
...
for i=1:1:size(remaining_points, 1)
    ...
    l2_dist_query_to_all_points = sum(l2_dists_query_to_all);
    if (l2_dist_query_to_all_points > dists_max_points_to_all_points(index_min))
        for j=1:1:k+1
            dists_max_points_to_all_points(j) = dists_max_points_to_all_points(j) + sqrt(sum((k_farthest_points(j, :) - query_point).^(2), 2)) - sqrt(sum((k_farthest_points(j, :) - k_farthest_points(index_min, :)).^(2), 2));
        end
        dists_max_points_to_all_points(index_min) = l2_dist_query_to_all_points;
